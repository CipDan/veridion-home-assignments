...
For every reported match, record the Companies House sample company number, source value, normalized VAT number, HMRC-returned target.name and address, matching rule, and unresolved mismatches. A successful HMRC response confirms VAT registration, but it does not prove that the VAT number belongs to the sampled company. Without this mapping, the report can associate a valid VAT number with the wrong company.
