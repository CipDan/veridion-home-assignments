...
For this project, you are provided with a global 3.14.7 Python environment accessible through the `py -3.14` command on Windows or `python3.14` on POSIX systems. To check available modules, invoke pip through the selected interpreter: use `py -3.14 -m pip list` on Windows or `python3.14 -m pip list` on POSIX systems, and use bare `python -m pip list` only when `python` itself resolves to this 3.14 environment; to facilitate some of the work that will need to be done, the environment comes with modules `pandas` (for sample or request-fetched data visualization), `requests` (for HTTP requests), `python-dotenv` (for reading key-value pairs from .env files) and `mypy` (for Python code type-checking) installed.
...
* __Local/environment paths:__ never quote a full local filesystem path that reveals machine- or account-identifying information (e.g. a Windows user-profile path like `C:\Users\<name>\...`) in any project file, including session snapshots in `docs`. Redact the identifying segment with a structured placeholder that keeps the rest of the path legible (e.g. `[HOME-DIRECTORY]\AppData\Local\Temp`); the exact placeholder wording is flexible, but the identifying segment must never appear in full. This applies even inside a block marked "verbatim" — sanitizing an identifying path takes precedence over verbatim fidelity, and does not count as altering a snapshot's historical narrative.
...
* The files in directory `docs` are session snapshots, historical narratives of the work that was done during specific sessions. That does not mean, however, that editorial errors cannot occur. As such, editing a snapshot is generally permitted, __but__ any edits to be made __MUST NOT__ conflict/alter the historical narrative of the session; if updates occurred to files/workflows recorded by a previous snapshot, these will be recorded in FINDINGS.md and __perhaps__ another snapshot following the one affected, and __NOT__ in said snapshot.
