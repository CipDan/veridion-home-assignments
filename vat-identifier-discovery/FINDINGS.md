...
- **Random sample of 89 distinct council organizations** (title-keyword filtered to exclude non-local-authority bodies that also contain "Council" in their name — British Council, Science and Technology Facilities Council, Research Councils, Higher Education Funding Council for England, Council for Healthcare Regulatory Excellence, General Social Care Council — none of which are local government):
  - **60/89 (67%) had no live CSV resource at all** in CKAN's own metadata — a dead/empty resource entry, not a reachability problem on our end.
  - **20/89 (22%) failed to fetch**: mostly `403 Forbidden` (the council's own site blocking scripted/non-browser access) or connection timeouts.
  - **2/89 (2%) resolved to a URL that served an HTML error/landing page instead of a CSV** (a broken/stale link CKAN still lists as CSV).
  - **Only 7/89 (8%) were successfully reached and parsed as real spend data.**
- **Of the 7 successfully-checked councils, 1 (Pendle Borough Council) had a column with "VAT" in its name** — but it is `Irrecoverable VAT (N)`, an accounting flag for VAT that can't be reclaimed on a given purchase, **not a supplier VAT registration number**. It was also empty in all 834 rows. **Zero of the 7 reachable councils had a genuine VAT-registration-number column.**
- **What this does and doesn't prove:** the negative result here is dominated by an *access* problem (92% of the random sample was unreachable through this route), not a clean measurement of "councils don't populate VAT data." The finding that should be trusted is narrower: **the data.gov.uk CKAN catalog is not a practically usable bulk-discovery route for this data** — its resource metadata is stale/broken for the large majority of listed council datasets. Whether individual councils' own (live, correctly-linked) transparency pages carry a VAT column more often than this sample suggests remains genuinely unresolved; it would need per-council direct crawling, a materially different (and much higher-effort) approach than this session's brief covered.
