...
- Reusable tooling built for this: `vat-identifier-discovery/scripts/hmrc_vat_check.py` (OAuth2 client-credentials flow, VAT lookup call, `is_valid_uk_vat_checksum()`), `csv_utils.py` (generic large-CSV column loading via pandas), `ocds_utils.py` (generic OCDS bulk-JSONL parsing), `peppol_utils.py` (PEPPOL Directory REST search client). Batch-specific driver scripts (`validate_find_a_tender.py`, `validate_peppol.py`, ...) import these rather than duplicating logic.
...
6. **Local council spend CSVs — does any council actually populate a VAT column?** LGA guidance recommends it; no live example confirmed yet (contrast with DEFRA, which is confirmed). Sample a handful of council transparency files.
